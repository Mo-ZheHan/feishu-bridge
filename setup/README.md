...
# Node（本机用 nvm；launchd plist 里写的是绝对路径 v20.20.2）
...
| 工具 | 路径 | 谁在用 |
|------|------|--------|
| `tmux` | `/opt/homebrew/bin/tmux` | 会话持久化、按键注入（launcher / remote-launch / zshrc） |
...
| `node` | `~/.nvm/.../v20.20.2/bin/node` | 监听器 + 所有 hook |
...
| `env.full.example` | `<仓库根>/.env` | 飞书凭据 + 运行时开关（含 `CLAUDE_REMOTE_BASE_OVERRIDE`） |
...
- **rsync 必须用 brew 版**：launcher/zshrc 都写死 `/opt/homebrew/bin/rsync`（3.4.3）；新版 macOS 自带的 `/usr/bin/rsync` 实为 `openrsync`，部分 GNU 选项不兼容。
- **`hpc` 项目在 home 根**：不在 `~/Code`，所以 zshrc 有 `CLAUDE_REMOTE_BASE_OVERRIDE=( [hpc]='~' )`，`.env` 里对应 `CLAUDE_REMOTE_BASE_OVERRIDE=hpc=~`。两边要一致。
- **远程主机表三处同源**：`zshrc-claude.zsh` 的 `CLAUDE_REMOTE_HOSTS`、`launcher.js` 的 `REMOTE_HOSTS`、`ssh-config` 的 `Host` 必须对齐。
- **mutagen 忽略是单一来源**：改 `~/.mutagen.yml`，zshrc 和 `remote-launch.js` 的 rsync exclude 会同时跟着变。
